...
#define FU_RTS54HUB_DEVICE_STATUS_LEN			32

#define FU_RTS54HUB_HUB_CMD_ENABLE			3
#define FU_RTS54HUB_HUB_CMD_DISABLE			1
#define FU_RTS54HUB_HUB_VENDOR_ENABLE			1
...
fu_rts54hub_device_to_string (FuDevice *device, GString *str)
{
	...
	g_string_append_printf (str, "    is-eeprom: %i\n", self->is_eeprom);
}
...
static gboolean
fu_rts54hub_device_vendor_cmd_disable (FuRts54HubDevice *self, GError **error)
{
	GUsbDevice *usb_device = fu_usb_device_get_dev (FU_USB_DEVICE (self));
	if (!g_usb_device_control_transfer (usb_device,
					    G_USB_DEVICE_DIRECTION_HOST_TO_DEVICE,
					    ...
					    FU_RTS54HUB_DEVICE_TIMEOUT,
					    NULL, error)) {
		g_prefix_error (error, "failed to cmd disable: ");
		return FALSE;
	}
	return TRUE;
}
...
static gboolean
fu_rts54hub_device_vendor_cmd_enable (FuRts54HubDevice *self,
				      guint8 operate_flash,
				      GError **error)
{
...
fu_rts54hub_device_setup (FuDevice *device, GError **error)
{
	...
	if (self->dual_bank && self->fw_auth && self->is_eeprom)
...
fu_rts54hub_device_close (FuUsbDevice *device, GError **error)
{
	...
	if (!fu_rts54hub_device_vendor_cmd_disable (self, error)) {
...
fu_rts54hub_device_write_firmware (FuDevice *device, GBytes *fw, GError **error)
{
	...
	/* enabe interface? */
	if (!fu_rts54hub_device_vendor_cmd_enable (self,
						   FU_RTS54HUB_HUB_VENDOR_ENABLE,
						   error)) {
		g_prefix_error (error, "failed to vendor enable: ");
		return FALSE;
	}
	if (!fu_rts54hub_device_vendor_cmd_enable (self,
						   FU_RTS54HUB_HUB_CMD_ENABLE,
						   error)) {
		g_prefix_error (error, "failed to cmd enable: ");
		return FALSE;
	}
	...
	if (!fu_rts54hub_device_highclockmode (self, 0x0001, error)) {
		g_prefix_error (error, "failed to enable clock #1: ");
		return FALSE;
	}
	if (!fu_rts54hub_device_highclockmode (self, 0x0101, error)) {
		g_prefix_error (error, "failed to enable clock #2: ");
		return FALSE;
	}
	...
	if (!fu_rts54hub_device_vendor_cmd_enable (self, 
						   FU_RTS54HUB_HUB_VENDOR_ENABLE,
						   error)) {
		g_prefix_error (error, "failed to enable vendor: ");
		return FALSE;
	}
